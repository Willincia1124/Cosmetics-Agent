...
- 如果能从网页搜索中找到明确商品页，会优先返回商品页
- 如果找不到稳定 SKU，会自动生成淘宝、天猫、京东的真实搜索入口
- 这保证了 demo 商品库阶段也能跳转到真实电商平台完成购买探索
- 平台展示的价格以淘宝 / 天猫 / 京东实时结果为准

说明：淘宝、京东的结构化商品搜索 API 通常需要开放平台或联盟账号、签名密钥和审核权限。当前项目先用真实电商搜索入口跑通闭环；后续拿到官方 API 后，可以把 `get_purchase_links` 替换为结构化 SKU、价格、销量和佣金链接。
